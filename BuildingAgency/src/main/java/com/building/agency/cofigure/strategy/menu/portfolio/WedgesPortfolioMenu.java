/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.building.agency.cofigure.strategy.menu.portfolio;

import com.building.agency.cofigure.app.ApplicationContext;
import com.building.agency.cofigure.app.CompanyContext;
import com.building.agency.cofigure.io.model.PortfolioModel;
import com.building.agency.cofigure.io.model.WedgesModel;
import com.building.agency.cofigure.menu.MenuItemModel;
import com.building.agency.utils.M;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

/**
 * Declare Wedges Portfolio Menu
 * @author nnastas
 * @version 1.0
 */
public class WedgesPortfolioMenu extends PortfolioMenu{
    
    private WedgesModel[] wedges = null; // serialized wedges are stored here
    
    /**
     * Default constructor for Wedges Portfolio Menu
     * @param context - application context
     * @version 1.0;
     * @throws java.io.IOException
     */
    public WedgesPortfolioMenu(ApplicationContext context) throws IOException {
        super(context, M.menus.WEDGES_PORTFOLIO_MENU);
        PortfolioModel porfolios = ((CompanyContext)context).getApplicationContentProvider().
                getFiguresPorfolio(); // portfolio figures a created here
        wedges = porfolios.getWedges(); // serialized pyramids are stored here
    }

    @Override
    public List<MenuItemModel> retrieveItems() {
        
        List<MenuItemModel> items = new ArrayList<>();
        int i = 1;
        for(WedgesModel wedge:wedges)
        {
           items.add(new MenuItemModel(i++, wedge.getName()));
        }
        items.add (new MenuItemModel(9, M.menu_items.BACK));
        items.add (new MenuItemModel(0, M.menu_items.QUIT));
        
      
        return items;  
    }
    
}
